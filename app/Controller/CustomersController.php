 ...
 */
    public function index() {
        ...
        if (!$this->RequestHandler->isAjax()) {
                            ...
                            if ($search_key != 'sort' && $search_key != 'direction' && $search_key != 'page') {
                                if (isset($search_value) && $search_value != '') {
                                    ...
                                    if (strstr($search_key, 'company_id')) {
                                        $sql = '';
                                        $sql .= "(`Customer`.`company_id`  = ${search_value}) OR ";
                                    ...
                                    if (strstr($search_key, 'members')) {
                                        $sql = '';
                                        $sql .= "(`Customer`.`member_id`  = ${search_value}) OR ";
                                        $sql .= "(`Customer`.`member2_id` = ${search_value}) OR ";
                                        $sql .= "(`Customer`.`member3_id` = ${search_value}) OR ";
                                        $sql .= "(`Customer`.`member4_id` = ${search_value}) OR ";
                                        $sql .= "(`Customer`.`member5_id` = ${search_value}) OR ";
                                    ...
                                    // Status
                                    if (strstr($search_key, 'status')) {
                                        $sql = '';
                                        $sql .= "(`Customer`.`status` LIKE '%${search_value}%') OR ";
                                        $sql = substr($sql, 0, -3);
                                    ...
                                    // Type
                                    if (strstr($search_key, 'type')) {
                                        $sql = '';
                                        $sql .= "(`Customer`.`type` LIKE '%${search_value}%') OR ";
                                        $sql = substr($sql, 0, -3);
            ...
            # 技術担当
            $option_mem = $this->Member->find('list', array(
                'fields' => array('Member.id', 'Member.name')
            ));
            $this->set('option_mem', $option_mem);

            # SIer
            $option_com = $this->Company->find('list', array(
                'fields' => array('Company.id', 'Company.company_name')
            ));
            $this->set('option_com', $option_com);
        }
    }
 ...
 */
    public function edit($id = null) {
        ...
        if ($this->request->is('post') || $this->request->is('put')) {
            ...
            if ($this->Customer->save($this->request->data)) {
                $this->Session->setFlash(__('案件情報の更新に成功しました。'), 'Flash/success');
                $this->redirect(array('action' => 'index'));
            } else {
                $this->Session->setFlash(__('案件情報の更新に失敗しました。'));
            }
        } else {
    ...
    public function ajaxList($ctp = null) {
        ...
        if ($this->RequestHandler->isAjax()) {
            foreach ($this->request->data as $search_key => $search_value) {
                if (isset($search_value) && $search_value != '') {
                    if (strstr($search_key, 'prefecture_id')) {
